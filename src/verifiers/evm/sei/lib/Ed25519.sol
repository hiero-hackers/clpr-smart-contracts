...
// Vendored from github.com/chengwenxi/Ed25519 (Apache-2.0), ported 0.6.8 -> 0.8.
// Pure-Solidity Ed25519 verification — used because Sei v6.5.2 has no EIP-665
// ed25519 precompile at 0x09 (that address is BLAKE2F on Sei).

import "./Sha512.sol";

/// @title Ed25519
/// @notice Pure-Solidity Ed25519 signature verification, used because Sei v6.5.2 has no EIP-665
///         ed25519 precompile (its `0x09` is BLAKE2F). This lets the verifier check the CometBFT
///         validators' commit signatures in EVM bytecode.
///
/// @dev Reader's map — all arithmetic is over the Curve25519 field prime `p = 2^255 - 19`
///      (the recurring `0x7fff..ffed` constant):
///        - `verify(k, m, s)` is the entry point: it implements the standard Ed25519 check
///          `[S]·B == R + [H(R‖A‖M)]·A`, returning true iff the signature `s` is valid for
///          public key `k` and message `m`.
///        - `pow22501` / `pow22523` raise a field element to a fixed exponent via an unrolled
///          square-and-multiply addition chain (hence the many near-identical `mulmod(a, a, p)`
///          rows — each is one squaring step, so a run of `k` computes `a^(2^k)`). They yield the
///          modular inverse and square root needed below.
///        - point decompression recovers the curve point `A` from the 32-byte public key
///          (solving for `x` from `y` using that square root).
///
///      The dense addition-chain sections are vendored verbatim from
///      github.com/chengwenxi/Ed25519 (Apache-2.0, see header) and kept as-is so they stay
///      auditable against the upstream / djb `ref10` reference. Behaviour is pinned by
///      `Ed25519Port.t.sol` plus a real on-chain signature replay vector.
library Ed25519 {
