...
The `<ticket_number>` should be the GitHub issue number for which the branch was created.

> **Deviation from the upstream standard:** the upstream convention zero-pads the ticket number to
> five digits. This repository does **not** pad — its issue numbers are used verbatim, matching the
> existing branches (`108-sei-verifier`, `142-connection-limits`, `319-sei-trust-anchor-rotation`).
...
`142-connection-limits`

`235-dispatch-onbound-msg`

Branches predating this policy do not all conform (some use a `<ticket>/<desc>` form, and a few
carry no ticket at all). They are left as-is; the standard applies to newly created branches.
...
This repository has no release branches today, but `refs/heads/release/**` is already covered by the
branch-protection rulesets, so the first one created is protected on day one.
...
All tags are covered by the `[Tag] Layer 1: Basic Limits` ruleset — creation, update, deletion, and
force-push are blocked, and tags must be signed.
